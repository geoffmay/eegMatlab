...
%     end
  ...
  if(doAsymmetry)
      for i = 1:length(pow)
          input1 = pow(i).absolutePower;
          for j = 1:length(pow)
              input2 = pow(j).absolutePower;
              for k = 1:size(input1,2)
                  output = input1(:,k) - input2(:,k);
                  labels{counter} = sprintf('asy %s-%s %dHz-%dHz', pow(i).label, pow(j).label, freqInfo.lowFrequencies(k), freqInfo.highFrequencies(k));
                  if(size(matrix(:,1), 1) == size(output,1))
                      if(~any(isnan(output)))
                          matrix(:, counter) = output;
                          counter = counter + 1;
