...
while(channelNumber <= length(testData.channels))
    ...
    while(freqNumber <= freqCount && channelNumber <= length(testData.channels))
        ...
        while(channelNumber <= length(testData.channels) && chunkChannelIndex <= chunkChannelCount)
            measureLabel = 'absolutePower';
            chanLabel = testData.channels(channelNumber).label;
            %for freqNumber = 1:freqCount
            while(freqNumber <= freqCount && chunkChannelIndex <= chunkChannelCount)
            ...
            if(chunkChannelIndex <= chunkChannelCount || freqNumber > freqCount)
                freqNumber = 1;
                channelNumber = channelNumber + 1;
            end
        end
