function [ phaseSlopeTopographies ] = phaseSlopeTimecourse2( EEG, outputFolder, frequencyLimits )
...
sampleSeries = 1:windowIncrement:(size(EEG.data,2) - windowSize + 1);
sampleSeriesPermutation = halvingSeries(sampleSeries);

%while(sampleCounter + windowSize < size(EEG.data,2))
for permIndex = 1:length(sampleSeriesPermutation)
    sampleCounter = sampleSeriesPermutation(permIndex);
    outputFile = fullfile(outputFolder, sprintf('phaseSlope-%06d.mat', sampleCounter));
    if(~exist(outputFile, 'file'))
        ...
        save(outputFile, 'phaseSlopeTopographies');
    end
...
for i = 1:length(phaseFiles)
    ...
    if(isfield(a, 'phaseSlopeTopographies'))
        if(i == 1)
            ...
            phaseSlopeTopographies.times = NaN(length(phaseFiles), 1);
            phaseSlopeTopographies.channels = EEG.chanlocs;
            phaseSlopeTopographies.frequencyMins = frequencyLimits(:,1);
            phaseSlopeTopographies.frequencyMaxes = frequencyLimits(:,2);
        end
