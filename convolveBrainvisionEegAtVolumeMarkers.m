function consolidated = convolveBrainvisionEegAtVolumeMarkers(edfFilename)
...
for fileCounter = 3:length(inputFiles)
    ...
    if(~exist(outputPath, 'file'))
        ...
        dSignal = downsample(piece.signal, downsampleRate);
        dRate = piece.freqInfo.samplesPerSecond / downsampleRate;
        
        dHrf = convolveHrf(dSignal, dRate);
        for i = 1:length(dMarkers)
            targetFrame = markers(i);
            targetTime = targetFrame / piece.freqInfo.samplesPerSecond;
            pieceFrame = min(find(piece.x >= targetTime));
            
            marker.timeSeconds = piece.x(pieceFrame);
            dFrame = pieceFrame / downsampleRate;
            marker.signal = interp1(1:length(dHrf), dHrf, dFrame);
            if(i == 1)
                volumeValues = repmat(marker, [1, length(markers)]);
            end
            volumeValues(i) = marker;
        end
