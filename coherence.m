function [ coherencePlot, phaseAnglePlot ] = coherence( data1, data2, lowFreq, highFreq)
...
while(fftEndIndex < length(data1))
  ...
  if(processFrequenciesInGroups)
    ...
    for i = 1:length(lowFreq)
      tempFft1(i) = mean(fft1((lowFreq(i)+1):highFreq(i)+1));
      tempFft2(i) = mean(fft2((lowFreq(i)+1):highFreq(i)+1));
    end
    fft1 = tempFft1;
    fft2 = tempFft2;
  end
