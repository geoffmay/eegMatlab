...
function [ summary ] = asymptoteSignalReliability( signal, frameCount, minDeltaFraction )
%RESAMPLECOHERENCERELIABILITY Summary of this function goes here
%   Detailed explanation goes here

if(~exist('minDeltaFraction', 'var'))
    minDeltaFraction = 0.001;
end
if(size(signal, 1) == 1)
    signal = signal';
elseif(size(signal, 1) < size(signal, 2))
    error('detected more columns than rows - make sure your data is shaped correctly, with a row for each timepoint and a column for each measure');
end

surfCoh.matrix = signal;
